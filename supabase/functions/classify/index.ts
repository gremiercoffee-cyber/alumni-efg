...
function settingsContext(settings: Record<string, string> = {}) {
  return [
...
function folderContext(folderList: FlatFolder[] = [], emptyLabel: string) {
  return folderList.length
    ? folderList.map(f => `- id: ${f.id} | path: ${f.path.join(' > ')}`).join('\n')
    : emptyLabel;
}

function todoContext(todoLists: TodoList[] = [], folderList: FlatFolder[] = []) {
...
- Output strictly valid JSON only.`;
  ...
  const { transcript, folderList, todoLists, settings } = body;
  const contextBlock = settingsContext(settings);
  const folderDesc = folderContext(folderList, '(no folders exist yet - this may be the first capture)');
...
Current server time: ${localNow.toString()}
...
For calendar-directed scheduling, use exactly this shape:
{
  ...
  "calendarEntries": [
    { "title": "short event title", "date": "upcoming day or date", "time": "specific time or a phrase like 'morning'", "durationMinutes": 60, "eventType": "meeting" }
  ]
}
...
- For calendar_entries, include eventType using one of: meeting, reminder, delivery, appointment, task, shiur, personal, other.
